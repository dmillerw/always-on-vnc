...
public class DesCipher {
    ...
    private static byte[] bytebit = {1, 2, 4, 8, 16, 32, 64, Byte.MIN_VALUE};
    ...
    public void encryptText(byte[] clearText, byte[] cipherText, byte[] key) {
        for (int i = 0; i < 8; i++) {
            clearText[i] = (byte) (clearText[i] ^ key[i]);
        }
        encrypt(clearText, 0, cipherText, 0);
        for (int i2 = 8; i2 < clearText.length; i2 += 8) {
            for (int j = 0; j < 8; j++) {
                int i3 = i2 + j;
                clearText[i3] = (byte) (clearText[i3] ^ cipherText[(i2 + j) - 8]);
            }
            encrypt(clearText, i2, cipherText, i2);
        }
    }

    public void decryptText(byte[] cipherText, byte[] clearText, byte[] key) {
        for (int i = cipherText.length - 8; i > 0; i -= 8) {
            decrypt(cipherText, i, clearText, i);
            for (int j = 0; j < 8; j++) {
                int i2 = i + j;
                clearText[i2] = (byte) (clearText[i2] ^ cipherText[(i + j) - 8]);
            }
        }
        decrypt(cipherText, 0, clearText, 0);
        for (int i3 = 0; i3 < 8; i3++) {
            clearText[i3] = (byte) (clearText[i3] ^ key[i3]);
        }
    }
